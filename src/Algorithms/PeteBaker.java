        ...
        public static int cakes(Map<String, Integer> recipe, Map<String, Integer> availableIngredients) {
                ...
                return Arrays.stream(recipe.keySet().toArray(String[]::new))
                                .mapToInt(ingredient -> (int) Math
                                                .floor((double) availableIngredients.getOrDefault(ingredient, 0)
                                                                / recipe.get(ingredient)))
                                // Use the min() function to handle the case where the min() function is called on an empty stream.
                                .min().orElse(Integer.MIN_VALUE);

        }

        public static void main(String[] args) {
                // Create an unmodified map to represent the recipe
